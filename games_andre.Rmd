...
ggplot(games_per_year,aes(x=as.numeric(year_of_release),y=game_per_year,group=as.numeric(year_of_release),color="random")) + geom_line()
```
...
ggplot(world_sales, aes(x=genre, y=sales)) + geom_bar(stat="identity", fill="yellow")
```
...
ggplot(world_sales, aes(x=, y=sales)) + geom_bar(stat="identity", fill="yellow")
```
...
```{r}
ggplot(gaming_no_na,aes(x=global_sales,y=))+ geom_smooth(method = "lm",se=FALSE)+
